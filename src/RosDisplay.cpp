...
void RosDisplay::publishBackendOutput(
...
void RosDisplay::publishMesherOutput(
...
//  If we make this a VisualizerModule we get this for free.
bool RosDisplay::publishSyncedOutputs() {
  ...
  if (backend_output_queue_.pop(backend_output)) {
    ...
    bool get_mesher =
        SimpleQueueSynchronizer<MesherOutput::Ptr>::getInstance().syncQueue(
            ts, &mesher_output_queue_, &mesher_output, "RosDisplay");
    if (mesher_output) {
    ...
    if (frontend_output && mesher_output) {
      ...
      if (image_publishers_->getNumSubscribersForPublisher("debug_img") > 0) {
        // TODO(Toni): this could be done in visualizer and sent to display
        // module.
...
void RosDisplay::publishLcdOutput(
...
void RosDisplay::publishTimeHorizonPointCloud(
...
void RosDisplay::publishDebugImage(
...
void RosDisplay::publishPerFrameMesh3D(
    const MesherOutput::Ptr& output) const {
  CHECK(output);
...
void RosDisplay::publishTf(const BackendOutput::Ptr& output) {
  CHECK(output);
...
void RosDisplay::publishFrontendStats(
    const FrontendOutput::Ptr& output) const {
  CHECK(output);
...
void RosDisplay::publishResiliency(
...
void RosDisplay::publishImuBias(
    const BackendOutput::Ptr& output) const {
  CHECK(output);
...
void RosDisplay::publishOptimizedTrajectory(
...
void RosDisplay::updateRejectedEdges() {
...
void RosDisplay::updateNodesAndEdges(
...
pose_graph_tools::PoseGraph RosDisplay::getPosegraphMsg() {
  // pose graph getter
...
void RosDisplay::publishPoseGraph(
    const LcdOutput::Ptr& lcd_output) {
  CHECK(lcd_output);
...
void RosDisplay::publishTf(const LcdOutput::Ptr& lcd_output) {
  CHECK(lcd_output);
